...
# The same on-disk shape as the autosave, so a saved design and an autosave file
# are interchangeable — but explicit: designs become committable artifacts you
# can ship (an example can carry its canvas), and an explicit load fails LOUDLY
# (a ValueError), unlike the autosave's warn-and-start-blank.


def save_design(path: Path | str) -> Path:
    """Write the current design to ``path`` (atomically). Raises ValueError when
    there's nothing to save yet."""
    from . import state

    project = state.get_project()
    if project is None:
        raise ValueError("no design to save yet — open the editor and build something first")
    path = Path(path)
    path.parent.mkdir(parents=True, exist_ok=True)
    tmp = path.with_suffix(path.suffix + ".tmp")
    tmp.write_text(json.dumps({"version": 2, "project": project.model_dump()}, indent=1))
    os.replace(tmp, path)
    return path


def load_design(path: Path | str) -> Project:
    """Read a design file, make it the current project (the autosave write-through
    fires, and open editor tabs sync live), and return it. A missing or invalid
    file raises with a clear message — an explicit load never silently no-ops."""
    from . import state

    path = Path(path)
    if not path.exists():
        raise ValueError(f"no design file at {path}")
    try:
        raw = json.loads(path.read_text())
        project = Project.model_validate(raw["project"])
    except Exception as exc:
        raise ValueError(f"{path} is not a valid design file ({exc})") from None
    state.set_project(project)
    # Mirror to open tabs (fire-and-forget; a no-op without a server/tab): the
    # same sync frame another tab's edit would produce, validation included.
    try:
        from .ws import _validate_project, manager

        models, code, links = _validate_project(project, False)
        manager.broadcast_threadsafe(
            {"type": "sync", "project": project.model_dump(), "models": models, "code": code, "links": links}
        )
    except Exception:
        pass
    return project
