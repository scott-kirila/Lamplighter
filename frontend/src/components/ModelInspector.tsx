...
// nameless model would flow into role dropdowns, run attribution, and the
// generated class name). Blur restores the last real name if left empty, and
// commits trimmed. External renames (the sidebar, a remote tab) re-seed the
// draft — the emitted-ref pattern the param editors use.
function RenameField({ name, onRename }: { name: string; onRename: (v: string) => void }) {
